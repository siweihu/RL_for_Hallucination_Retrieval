...
class PPOTrainer(BaseTrainer):
    ...
    def generate(
        ...
        """
        # Remove all other if, else choice that check whether the query_tensor is list by hsw

        if generate_ref_response:
            ref_model = self.model if self.is_peft_model else self.ref_model

        # if isinstance(query_tensor, List):
        ## change the generate of PPO to the generate of generative retrieval by hsw
        ...
        response = outs.sequences  # [B*K, T]
        resp_scores = outs.sequences_scores
        ...
        if generate_ref_response:
            with self.optional_peft_ctx():
                ...
                #     **generation_kwargs,
                # )
                outs_ref = self.ref_model.generate(
                    query_tensor,
                    max_length=20,
                    num_beams=20,
                    prefix_allowed_tokens_fn=restrict_decode_vocab,
                    ...
                    early_stopping=True,
                    **generation_kwargs,)
                ref_response = outs_ref.sequences
                ref_scores = outs_ref.sequences_scores
        ...
        if generate_ref_response:
            return response, resp_scores, ref_response, ref_scores
        return response, resp_scores
...
#         print(vpreds)
    ...
    def compute_rewards(
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            # change the first mask to 1 since our response is just 1
            mask[0] = 1
