    ...
    def inference_per_example(self, premise:str, hypo: str):
        ...
        if self.nlg_eval_mode is not None:
            if self.nlg_eval_mode == 'nli_sp':
                result = self.inference(premise_sent_mat, hypo_sents_mat)[2]

                ## overlap
                reward_overlap = []
                for sss in range(len(result)):
                    keywords = custom_kw_extractor.extract_keywords(hypo_sents_mat[sss])
                    keywords = [t[0] for t in keywords]

                    num_entail = 0
                    for j in range(len(keywords)):
                        keyword = keywords[j]
                        if keyword in premise_sent_mat[sss]:
                            num_entail += 1
                            if j < 5:
                                num_entail += 1

                    reward_overlap.append(num_entail / 15)

                # output_score = result[:, 0]### use NLI head OR ALIGN head
                # print(result)

                ### reward score by hsw
                # coefficent = [1.5, 0.2, 1]
                output_score = []
                for num in range(len(result)):
                    values = []
                    for i in range(3):
                        values.append(result[num, i])

                    max_value = max(values)
                    max_index = values.index(max_value)
                    high_reward = coefficent[max_index] * (math.log(max_value, 2)+1)
                    high_reward = 1.5*values[0] + values[2] + 0.2*values[1]
                    # random_entropy = -1*((values[2]+values[0]) * math.log((values[2]+values[0]), 2) + values[1] * math.log(values[1],2))
                    ...
                    q = np.array([0.5, 0.5])  # 目标分布 [0.5, 0.5]

                    random_entropy = jensen_shannon_divergence([values[0]+values[2],values[1]], q)

                    output_score.append(high_reward + 0.5*random_entropy)
    ...
    def inference(self, premise, hypo):
        ...
        for mini_batch in tqdm(batch, desc="Evaluating", disable=not self.verbose or self.disable_progress_bar_in_inference):
            ...
            with torch.no_grad():
                ...
                # print('reg:')
                # print(model_output_reg)
                # print('2-way:')
                # print(model_output_bin)
                # print('3-way:')
                # print(model_output_tri)
                # print('***********************************************************')
                # We mainly focus on the task that detects the hallucination degree of a claim when given the context. We believe this
