...
#         fast_tokenizer = T5TokenizerFast.from_pretrained(run_args.model_name, cache_dir='cache')
        if run_args.model_path:
            ...
            model = T5ForConditionalGeneration.from_pretrained(run_args.model_name, from_tf=True, cache_dir='cache')
...
#             compute_metrics=make_compute_metrics(fast_tokenizer, train_dataset.valid_ids),
        ...
        # save the models
        model.save_pretrained('./checkpoint_ablation')
        tokenizer.save_pretrained('./checkpoint_ablation')
